...
interface Props {
  ...
  setRecords?: React.Dispatch<React.SetStateAction<ReceiptRecord[]>>
  invoiceVouchers?: any[]
}
...
export function Receipt({ contacts, records = [], setRecords, invoiceVouchers = [] }: Props) {
  // ── Navigation States ────────────────────────────────────────────────────────
  ...
  // Helper: Calculate unpaid balance of a specific invoice
  const getUnpaidInvoiceBalance = (vou: any) => {
    const totalBilled = (vou.items || []).reduce((s: number, it: any) => s + (Number(it.saleAmount) || 0), 0)
    const totalPaid = records.reduce((s: number, r: any) => {
      // Skip the currently viewed receipt if editing/viewing it
      if (isViewMode && r.id === activeRecord?.id) return s
      ...
      const matchedPaid = lines.reduce((ls: number, l: any) => {
        if (l.invoiceNo === vou.voucherNo) {
          return ls + (Number(l.amount) || 0)
        }
        return ls
      }, 0)
      return s + matchedPaid
    }, 0)

    // Deduct draft lines of the current voucher too if they are mapped to this invoice
    const draftPaid = draftLines.reduce((s: number, l: any) => {
      if (l.invoiceNo === vou.voucherNo) {
    ...
    return totalBilled - totalPaid - draftPaid
  }

  // Filter invoices for selected customer with remaining outstanding balance
  const customerInvoices = invoiceVouchers.filter(v => {
    if (v.accountName !== party) return false
    const unpaid = getUnpaidInvoiceBalance(v)
    return unpaid > 0
  })
  ...
  useEffect(() => {
    if (isViewMode && activeRecord) {
      ...
      // Auto-upgrade legacy single-line receipts to the new lines array format on the fly
        ...
        setDraftLines([
          {
            ...
            invoiceNo: (activeRecord as any).invoiceNo || undefined,
            amount: legacyAmt,
  ...
  const handleJumpToVoucher = () => {
    ...
    if (idx >= 0 && idx <= records.length) {
      ...
      alert(`Receipt out of range (Available range: RCT:00001 to RCT:${padNo(records.length + 1)})`)
    }
  }

  const handleInvoiceChange = (vouNo: string) => {
    setInvoiceNo(vouNo)
    if (!vouNo) {
      setLineAmount('')
      return
    }
    const matchedVou = invoiceVouchers.find(v => v.voucherNo === vouNo)
    if (matchedVou) {
      const remaining = getUnpaidInvoiceBalance(matchedVou)
      setLineAmount(String(remaining))
    }
  }
  ...
  const handleAddLine = (e: React.FormEvent) => {
    ...
    // Verify if amount exceeds unpaid balance of linked invoice
    if (invoiceNo) {
      const matchedVou = invoiceVouchers.find(v => v.voucherNo === invoiceNo)
      if (matchedVou) {
        const remaining = getUnpaidInvoiceBalance(matchedVou)
        if (amt > remaining) {
          alert(`Amount (PKR ${amt.toLocaleString()}) exceeds invoice remaining balance (PKR ${remaining.toLocaleString()}).`)
          return
        }
      }
    }
  ...
  const handleSaveVoucher = () => {
    ...
    if (draftLines.length === 0) {
      alert('Please add at least one receipt line to the voucher.')
      return
    }
    ...
    const nextNo = `RCT:${padNo(records.length + 1)}`
  ...
  const handlePreview = () => {
    ...
    const popup = window.open('', `ReceiptPreview_${r.voucherNo}`, 'width=900,height=700,resizable=yes')
    if (!popup) return
    ...
    const linesHtml = r.lines.map((l, idx) => `
      <tr>
        ...
        <td style="padding:10px; border-bottom:1px solid #e4e4e7; font-family:monospace; font-weight:bold;">${l.invoiceNo || '—'}</td>
        <td style="padding:10px; border-bottom:1px solid #e4e4e7; color:#52525b;">${l.particulars || '—'}</td>
        <td style="padding:10px; border-bottom:1px solid #e4e4e7; text-align:right; font-weight:700; color:#16a34a;">PKR ${l.amount.toLocaleString('en-PK')}</td>
      </tr>
    `).join('')
    ...
    popup.document.write(`
      <html>
        <head>
          <title>Receipt Voucher Print - ${r.voucherNo}</title>
          <style>
            ...
            .voucher-title { font-size:16px; font-weight:bold; text-transform:uppercase; color:#16a34a; }
          ...
          <div class="doc">
            <div class="doc-header">
              ...
              <div style="text-align:right;">
                <div class="voucher-title">Cash Receipt Voucher</div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Received From Account (Party)</th>
                  <th>Offset Sales Invoice</th>
              ...
              <tfoot>
                <tr>
                  <td colspan="4" style="padding:12px 10px; font-size:10px; text-transform:uppercase;">Total Voucher Amount</td>
                  <td style="padding:12px 10px; text-align:right; font-size:13px; font-weight:900; color:#16a34a;">PKR ${totalVoucherAmount.toLocaleString('en-PK')}</td>
                </tr>
              </tfoot>
            </table>
    ...
    <div className="flex-1 flex flex-col overflow-hidden space-y-4 font-sans text-xs text-[#09090b]">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-[#e4e4e7]">
        <div className="flex items-center gap-3">
          <h2 className="font-bold text-[11px] uppercase tracking-widest text-[#09090b] font-sifonn">Cash Receipt</h2>
          
          <div className="flex items-center bg-[#fafafa] border border-[#e4e4e7] rounded-md px-2 py-0.5">
            <span className="font-bold text-[10px] text-[#71717a] mr-1 uppercase">RCT:</span>
            <input
              type="text"
              value={jumpNum}
      ...
      <div className="p-4 border border-[#e4e4e7] rounded-xl bg-[#fafafa]">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-[9px] font-bold text-[#71717a] mb-1 uppercase tracking-wider">Receipt Date</label>
            <input
              type="date"
              required
              disabled={isViewMode}
      ...
      {!isViewMode && (
        <form onSubmit={handleAddLine} className="p-4 border border-[#e4e4e7] rounded-xl bg-white space-y-3 shadow-xs">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
            <div>
              <label className="block text-[9px] font-bold text-[#71717a] mb-1 uppercase tracking-wider">Received From Account</label>
              <select
                value={party}
                onChange={(e) => {
            ...
            <div>
              <label className="block text-[9px] font-bold text-[#71717a] mb-1 uppercase tracking-wider">Offset Sales Invoice</label>
              <select
                value={invoiceNo}
                onChange={(e) => handleInvoiceChange(e.target.value)}
                disabled={!party}
                className="w-full h-9 border border-[#e4e4e7] px-3 rounded-md text-xs bg-white disabled:bg-neutral-100 focus:outline-none focus:border-black font-semibold font-mono"
              >
                <option value="">— Mapped Outstanding —</option>
                {customerInvoices.map(v => {
                  const unpaid = getUnpaidInvoiceBalance(v)
                  return (
                    <option key={v.voucherNo} value={v.voucherNo}>
                      {v.voucherNo} - Unpaid: PKR {unpaid.toLocaleString('en-PK')}
                    </option>
                  )
                })}
              </select>
            ...
            <div className="col-span-2 md:col-span-1">
              ...
              <input
                type="text"
                placeholder="e.g. Offset for partial bill"
        ...
        <table className="w-full border-collapse text-left text-xs text-[#09090b]">
          <thead className="bg-[#fafafa] border-b border-[#e4e4e7] font-semibold text-[#71717a] uppercase tracking-wider sticky top-0 z-10 text-[9px]">
            <tr>
              ...
              <th className="p-3">Received From Account (Credit)</th>
              <th className="p-3 w-36 font-mono">Offset Sales Invoice</th>
          ...
          <tbody className="divide-y divide-[#e4e4e7]">
            {draftLines.length === 0 ? (
              <tr>
                <td colSpan={isViewMode ? 5 : 6} className="p-8 text-center text-[#71717a] font-medium py-10">
              ...
              draftLines.map((l, index) => (
                <tr key={index} className="hover:bg-neutral-50 font-medium">
                  ...
                  <td className="p-3 font-mono">
                    {l.invoiceNo ? (
                      <span className="inline-flex items-center h-5 px-1.5 bg-neutral-100 border border-neutral-200 rounded font-bold text-[9px] text-[#71717a]">
                        {l.invoiceNo}
                      </span>
                    ) : (
          ...
          {draftLines.length > 0 && (
            <tfoot>
              <tr className="bg-[#fafafa] font-bold border-t border-[#e4e4e7] uppercase text-[10px] text-[#09090b]">
                ...
                <td className="p-3">Total Receipt Sum</td>
                <td className="p-3 text-right font-mono text-green-700 text-xs">
        ...
        <table className="w-full border-collapse text-left text-[10px] text-[#71717a]">
          <thead className="bg-[#fafafa] border-b border-[#e4e4e7] font-bold uppercase tracking-wider sticky top-0 z-10 text-[9px]">
            <tr>
              ...
              <th className="p-2 w-24">Receipt No</th>
              <th className="p-2 w-28">Method</th>
              <th className="p-2">Accounts Credited</th>
            ...
            {records.map((r, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-2 font-extrabold text-[#09090b]">{r.voucherNo}</td>
                  <td className="p-2 font-medium">{r.mode}</td>
                  <td className="p-2 truncate max-w-[200px]" title={partyList}>{partyList}</td>
                  <td className="p-2 text-right font-bold text-green-700 font-mono">
