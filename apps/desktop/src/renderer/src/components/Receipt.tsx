...
interface ReceiptRecord { id: number; party: string; amount: string; mode: string; date: string }
    ...
    <div className="flex-1 flex flex-col overflow-hidden space-y-4">
      <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-3 pb-4 border-b border-[#e4e4e7]">
        <div className="flex-1 min-w-[200px]">
          <label className="block text-[9px] font-bold text-[#71717a] mb-1 uppercase tracking-wider">Received From</label>
        ...
        <div className="flex-1 min-w-[140px]">
          <label className="block text-[9px] font-bold text-[#71717a] mb-1 uppercase tracking-wider">Amount</label>
          <input type="number" required placeholder="e.g. 500" value={amount} onChange={e => setAmount(e.target.value)}
        ...
        <button type="submit" className="h-9 px-5 bg-[#54e0e7] text-[#09090b] font-bold text-xs rounded-md hover:bg-[#3cd5dc] transition-all cursor-pointer shadow-sm shrink-0">Post Receipt</button>
      </form>
      <div className="flex-1 overflow-auto border border-[#e4e4e7] rounded-xl bg-white">
        <table className="w-full border-collapse text-left text-xs text-[#09090b]">
          <thead className="bg-[#fafafa] border-b border-[#e4e4e7] font-semibold text-[#71717a] uppercase tracking-wider sticky top-0 z-10">
            <tr>
              <th className="p-3">Received From</th><th className="p-3">Amount Received</th>
          ...
          <tbody className="divide-y divide-[#e4e4e7]">
            {records.map(r => (
              <tr key={r.id} className="hover:bg-[#fafafa]">
                ...
                <td className="p-3 font-semibold text-green-600">{r.amount}</td>
